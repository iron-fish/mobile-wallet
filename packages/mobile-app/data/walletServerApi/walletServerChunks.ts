...
class WalletServerChunks {
  private async getChunksDir(network: Network): Promise<string> {
    const directory =
      FileSystem.cacheDirectory + `chunks/${network.toString()}/`;
    try {
  ...
  async getChunksManifest(network: Network): Promise<ChunksManifest> {
    ...
    if (manifestInfo.exists) {
      const manifestString = await FileSystem.readAsStringAsync(manifestFile, {
    ...
    const result = await FileSystem.downloadAsync(
      LIGHT_BLOCKS_URLS[network] + "manifest.json",
      manifestFile,
    );
  ...
  async getChunkBlocks(network: Network, chunk: Chunk): Promise<void> {
    ...
    const result = await FileSystem.downloadAsync(
      LIGHT_BLOCKS_URLS[network] + chunk.blocks,
      directory + file,
    );
  ...
  async getChunkByteRanges(network: Network, chunk: Chunk): Promise<void> {
    ...
    const result = await FileSystem.downloadAsync(
      LIGHT_BLOCKS_URLS[network] + chunk.byteRangesFile,
      directory + file + ".gz",
    );
    if (result.status !== 200) {
      console.warn("Unhandled status code", result.status);
  ...
  async getChunkBlockAndByteRanges(network: Network, chunk: Chunk) {
    await Promise.all([
      this.getChunkBlocks(network, chunk),
      this.getChunkByteRanges(network, chunk),
    ]);
  }

  async readChunkBlocks(
    network: Network,
    chunk: Chunk,
    ...
    for (; pos < ranges.length; pos += blocksPerRead) {
      ...
      for (let i = pos; i <= endIndex; i++) {
        const block = LightBlock.decode(
          bytes.subarray(ranges[i][1] - start, ranges[i][2] - start + 1),
        );
        await onBlock(block);
      }
    }
