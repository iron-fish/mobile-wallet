...
export class RecentBlocks {
  ...
  private static async getRecentBlocksDir(network: Network): Promise<string> {
    const directory =
      FileSystem.documentDirectory + `latest-blocks/${network.toString()}/`;
    try {
  ...
  static async writeRecentBlock(
    ...
    const files = (await FileSystem.readDirectoryAsync(directory)).filter(
      (file) => file.endsWith(".block"),
    );
  ...
  static async getRecentBlock(
    network: Network,
    sequence: number,
  ): Promise<LightBlock | null> {
    const directory = await this.getRecentBlocksDir(network);

    const fileInfo = await FileSystem.getInfoAsync(
      directory + `${sequence}.block`,
    );
    if (!fileInfo.exists) {
      return null;
    }
