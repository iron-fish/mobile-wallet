...
import { z } from "zod";
import { Account, AccountSettings, Output, Transaction, TransactionType, WalletHandlers, WalletStatus } from "./types";
import { AccountFormat, LanguageKey, TransactionStatus } from "@ironfish/sdk";
import { generateKey } from "ironfish-native-module";

const ACCOUNTS: Account[] = [
  { name: "alice", publicAddress: "alice", head: null, viewOnly: false, balances: { 
    iron: {
  ...
  { name: "bob", publicAddress: "bob", head: null, viewOnly: true, balances: { 
    iron: {
      assetId: "51f33a2f14f92735e562dc658a5639279ddca3d5079a6d1242b2a588a9cbf44c",
      available: "0",
      confirmed: "0",
  ...
  { name: "carol", publicAddress: "carol", head: null, viewOnly: false, balances: { 
    iron: {
      assetId: "51f33a2f14f92735e562dc658a5639279ddca3d5079a6d1242b2a588a9cbf44c",
      available: "0",
      confirmed: "0",
...
const WALLET_STATUS: WalletStatus = { status: 'SYNCING', latestKnownBlock: 523142 }
...
export const walletDemoHandlers = f.facade<WalletHandlers>({
  createAccount: f.handler.mutation(async ({ name }: { name: string }) => {
    const k = generateKey();
    const account: Account = { name, publicAddress: k.publicAddress, head: null, balances: { 
      iron: {
        assetId: "51f33a2f14f92735e562dc658a5639279ddca3d5079a6d1242b2a588a9cbf44c",
        available: "0",
        confirmed: "0",
      ...
      custom: []
    }, viewOnly: false, settings: { balanceAutoHide: false } }
  ...
  getTransaction: f.handler.query(async ({ accountName, hash }: { accountName: string; hash: string; }): Promise<Transaction> => {
    return { 
      ...
      assetBalanceDeltas: [{ assetId: "51f33a2f14f92735e562dc658a5639279ddca3d5079a6d1242b2a588a9cbf44c", delta: "20" }],
      fee: "1",
      expiration: 0,
      blockHash: "00000000000004e987a7404874bf27beee12180aef982a3f730981ac25ffe642",
      submittedSequence: 0,
      ...
      notes: [{
        ...
        value: "20",
        spent: false,
        owner: "3f6d1bc503f78729c3b203b3ed28791d25e4d8e54cc803f06805749a1424880e",
        sender: "7e5070ea40b3ff7a8c54697084abed1b4dd86c026235eb8d23e6412431a36833",
      }],
      spends: [],
    };
  }),
  ...
  importAccount: f.handler.mutation(async ({ account, name }: { account: string, name: string }) => {
    const k = generateKey()
    const importedAccount: Account = { name, publicAddress: k.publicAddress, viewOnly: false, head: null, balances: { 
      iron: {
        assetId: "51f33a2f14f92735e562dc658a5639279ddca3d5079a6d1242b2a588a9cbf44c",
        available: "0",
        confirmed: "0",
  ...
  setAccountSettings: f.handler.mutation(async ({ name, settings }: { name: string, settings: Partial<AccountSettings> }) => {
    const account = ACCOUNTS.find((a) => a.name === name);
    if (!account) {
      throw new Error("No account found");
    }
